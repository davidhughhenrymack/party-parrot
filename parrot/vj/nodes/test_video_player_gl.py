...
class TestVideoPlayerGL:
    ...
    def test_video_player_no_videos_fallback(self, gl_context, color_scheme):
        ...
        video_node = VideoPlayer(
            fn_group="nonexistent", video_group="test", width=256, height=256
        )
        ...
        try:
            ...
            # Should have the specified dimensions
            ...
            print(
                f"No videos fallback - Dimensions: {result_framebuffer.width}x{result_framebuffer.height}"
            )
    ...
    def test_video_player_initialization_sizes(self, gl_context, color_scheme):
        ...
        video_node = VideoPlayer(
            fn_group="bg", video_group="test_group", width=320, height=240
        )
        ...
        try:
            ...
            # Check that it has the specified dimensions
            assert result_framebuffer.width == 320
            assert result_framebuffer.height == 240
            ...
            print(
                f"Video player default - Dimensions: {result_framebuffer.width}x{result_framebuffer.height}"
            )
    ...
    def test_video_player_custom_dimensions(self, gl_context, color_scheme):
        """Test VideoPlayer with custom width and height"""
        test_sizes = [(128, 128), (640, 480), (1280, 720)]

        for width, height in test_sizes:
            video_node = VideoPlayer(fn_group="bg", width=width, height=height)
            ...
            try:
                ...
                # Should match the specified dimensions exactly
                ...
                print(f"Custom dimensions test - {width}x{height}: OK")

            finally:
                video_node.exit()

    def test_video_player_aspect_ratio_handling(self, gl_context, color_scheme):
        """Test VideoPlayer aspect ratio calculations"""
        video_node = VideoPlayer(fn_group="bg", width=800, height=600)

        # Test the scaling calculation method
        video_node.video_width = 1920
        video_node.video_height = 1080
        video_node._calculate_scaling()

        # For 1920x1080 -> 800x600, video is wider (16:9 vs 4:3)
        # Should scale by height and crop sides
        expected_scale = 600 / 1080  # Scale by height
        assert abs(video_node.scale_factor - expected_scale) < 0.001

        # Test with a taller video
        video_node.video_width = 480
        video_node.video_height = 640
        video_node._calculate_scaling()

        # For 480x640 -> 800x600, video is taller (3:4 vs 4:3)
        # Should scale by width and crop top/bottom
        expected_scale = 800 / 480  # Scale by width
        assert abs(video_node.scale_factor - expected_scale) < 0.001
