...
class TestHotSparksEffect:
    ...
    def test_initialization_defaults(self):
        ...
        assert sparks.num_sparks == 400
        assert sparks.spark_lifetime == 3.0
        assert sparks.spark_speed == 0.8
        assert sparks.signal == FrameSignal.small_blinder
    ...
    def test_initialization_custom_params(self):
        ...
        assert sparks.spark_speed == 1.2
        assert sparks.signal == FrameSignal.pulse

    def test_generate_vibe_energetic(self):
        """Test that generate adjusts parameters for energetic vibe"""
        sparks = HotSparksEffect()
        vibe = Vibe(Mode.rave)
    ...
    def test_generate_vibe_chill(self):
        """Test that generate adjusts parameters for chill vibe"""
        sparks = HotSparksEffect()
        vibe = Vibe(Mode.chill)
        sparks.generate(vibe)

        assert sparks.num_sparks == 200
        assert sparks.spark_speed == 0.5
    ...
    def test_fragment_shader_contains_required_elements(self):
        ...
        assert "uniform vec3 spark_color" in shader
    ...
    def test_set_effect_uniforms_basic(self):
        ...
        sparks = HotSparksEffect(num_sparks=30, spark_lifetime=1.0, spark_speed=0.5)
    ...
    def test_set_effect_uniforms_uses_color_scheme(self):
        """Test that spark color comes from color scheme foreground"""
        sparks = HotSparksEffect()
    ...
    def test_pulse_detection(self):
        ...
        frame1 = Frame({FrameSignal.small_blinder: 0.0})
    ...
    def test_pulse_cooldown(self):
        ...
        # Start emission
        frame1 = Frame({FrameSignal.small_blinder: 0.8})
        sparks._set_effect_uniforms(frame1, self.mock_scheme)
        emission_time_1 = sparks.emission_start_time
        assert sparks.is_emitting

        # Signal stays high (emission should continue with same start time)
        frame2 = Frame({FrameSignal.small_blinder: 0.9})
        sparks._set_effect_uniforms(frame2, self.mock_scheme)
        emission_time_2 = sparks.emission_start_time
