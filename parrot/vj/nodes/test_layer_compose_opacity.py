...
class TestLayerComposeOpacity:
    ...
    def test_opacity_normal_blending_pixel_verification(
            ...
            try:
                ...
                if result_framebuffer:
                    ...
                    if opacity == 1.0:
                        ...
                        assert (
                            center_pixel[1] < 50
                        ), f"Green channel should be low, got {center_pixel[1]}"
                        assert (
                            center_pixel[2] < 50
    ...
    def test_opacity_additive_blending_pixel_verification(
        ...
        red_layer = StaticColor(
            color=(0.5, 0.0, 0.0), width=1280, height=720
        )  # Half-intensity red
        black_layer = StaticColor(
            color=(0.0, 0.0, 0.0), width=1280, height=720
        )  # Solid black
        ...
        layer_compose = LayerCompose(
            LayerSpec(black_layer, BlendMode.NORMAL),  # Base layer: black
            LayerSpec(
                red_layer, BlendMode.ADDITIVE, opacity=0.5
            ),  # Red with 50% opacity, additive
        )
        ...
        try:
            ...
            if result_framebuffer:
                ...
                # Expected: black + (red * 0.5) = 0 + (0.5 * 255 * 0.5) = 64
                expected_red = int(0.5 * 255 * 0.5)
                tolerance = 30
                assert (
                    abs(center_pixel[0] - expected_red) < tolerance
                ), f"Red channel should be ~{expected_red}, got {center_pixel[0]}"
