...
class TestMotionstripRenderer:
    ...
    def test_pan_rotation_at_zero(self):
        """Pan value 0 → +90° (renderer flips sign so DMX 0 swings to audience-left)."""
    ...
    def test_pan_rotation_at_max(self):
        """Pan value 255 → -90° (renderer flips sign so DMX 255 swings to audience-right)."""
        fixture = Motionstrip38(patch=1, pan_lower=0, pan_upper=255, invert_pan=False)
        fixture.set_pan(255)
        renderer = MotionstripRenderer(fixture, room_renderer=None)
    ...
    def test_pan_rotation_with_invert(self):
        ...
        # (with the flipped convention: high-DMX → negative rotation,
        # low-DMX → positive rotation, so the signs swap on invert).
    ...
    def test_pan_rotation_with_custom_range(self):
        ...
        # Set pan to minimum of range → +90° (flipped convention)
        fixture.set_pan(0)
        renderer = MotionstripRenderer(fixture, room_renderer=None)
        assert renderer._get_pan_rotation() == pytest.approx(90.0, abs=0.1)

        # Set pan to maximum of range → -90°
        fixture.set_pan(255)
        renderer = MotionstripRenderer(fixture, room_renderer=None)
        assert renderer._get_pan_rotation() == pytest.approx(-90.0, abs=0.1)
