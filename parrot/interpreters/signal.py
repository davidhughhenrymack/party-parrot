...
class SignalSwitchInterpreter(InterpreterBase[T]):
    def __init__(
        self,
        interp_std: InterpreterBase[T],
        responds_to: dict[FrameSignal, bool] | None = None,
    ):
        super().__init__(interp_std.group, interp_std.interpreter_args)
        self.interp_std = interp_std
        self.args = interp_std.interpreter_args
    ...
    def step(self, frame: Frame, scheme: ColorScheme):
        ...
        if self.responds_to.get(FrameSignal.big_blinder, False):
            if frame[FrameSignal.big_blinder] > 0.5:
                ...
                self.big_blinder_dimmer = self.big_blinder_dimmer * BIG_BLINDER_DECAY
...
def signal_switch(
    ...
    class SignalSwitch(SignalSwitchInterpreter[T]):
        def __init__(
            self,
            group: List[T],
            args: InterpreterArgs,
        ):
            super().__init__(interp_std(group, args))

    return SignalSwitch


def wrap_signal_switch(
    interpreter: InterpreterBase[T],
    enabled_signals: tuple[FrameSignal, ...],
) -> SignalSwitchInterpreter[T]:
    responds_to = {signal: False for signal in SIGNAL_PROBABILITIES}
    for signal in enabled_signals:
        responds_to[signal] = True
    return SignalSwitchInterpreter(interpreter, responds_to)
